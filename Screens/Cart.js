...
const CartScreen = () => {
  ...
  const fetchCartItems = async id => {
    try {
      const response = await fetch(`http://192.168.0.4:3000/cart/${id}`);
  ...
  const handleUpdateQuantity = async (productId, newQuantity) => {
    ...
    try {
      const response = await fetch('http://192.168.0.4:3000/cart/update', {
        method: 'PUT',
  ...
  const handleRemoveItem = async productId => {
    try {
      const response = await fetch('http://192.168.0.4:3000/cart/remove', {
        method: 'DELETE',
        headers: {'Content-Type': 'application/json'},
