...
const OrderScreen = ({ navigation }) => {
  ...
  const statusMap = {
    'Tất cả': (id) => `http://192.168.3.106:3000/donhang/user/${id}`,
    'Chờ xác nhận': (id) =>
      `http://192.168.3.106:3000/donhang/user/${id}/status?status=${encodeURIComponent('Chờ xác nhận')}`,
    'Chờ vận chuyển': (id) =>
      `http://192.168.3.106:3000/donhang/user/${id}/status?status=${encodeURIComponent('Chờ vận chuyển')}`,
    'Đang vận chuyển': (id) =>
      `http://192.168.3.106:3000/donhang/user/${id}/status?status=${encodeURIComponent('Đang vận chuyển')}`,
    'Đã hủy': (id) =>
      `http://192.168.3.106:3000/donhang/user/${id}/status?status=${encodeURIComponent('Đã hủy')}`,
  };
  ...
  const cancelOrder = async (orderId) => {
    try {
      const response = await fetch(`http://192.168.3.106:3000/donhang/${orderId}`, {
        method: 'PUT',
